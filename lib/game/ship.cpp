...
size_t Ship::GetShipSize() {
  const std::bitset<8> mask("00001110");
...
bool Ship::GetStateShot(const uint64_t& x, const uint64_t& y) { 
  const std::bitset<8> mask("11110000");
...
bool Ship::IsHit(const uint64_t& x, const uint64_t& y) {
  const size_t kHitOffset = 4;
  if (x == x_ && y == y_) {
    states_[kHitOffset] = 0;
    return true;
  }
  if (GetRotate() == 0) {
    if (x == x_ && y_ <= y && y < y_ + GetShipSize()) {
      states_[kHitOffset + y - y_] = 0;
      return true;
    }
  } else {
    if (y == y_ && x_ <= x && x < x_ + GetShipSize()) {
      states_[kHitOffset + x - x_] = 0;
      return true;
    }
  }
  return false;
